...
Give every field a schema default (zod `.default()`) and call `Form` with just the schema — initial values come straight from the schema. Pass `{ defaultValues }` in options when you need explicit ones. The factory returns an ilha island with the schema captured in its closure, so the schema is never serialized through `data-ilha-props`.

<Preview
  code={
    'import { ilha } from "ilha";\nimport { z } from "zod";\nimport { Form } from "@areia/form";\n\nconst schema = z.object({\n  name: z.string().default("John"),\n  age: z.number().min(0).max(120).default(30),\n  theme: z.enum(["light", "dark", "system"]).default("system"),\n  active: z.boolean().default(true),\n});\n\nconst MyForm = Form(schema, {\n  onSubmit: (values) => console.log(values),\n});\n\nexport default ilha(() => <MyForm />);'
  }
  lang="tsx"
>
  <Demo1 client:load />
...
export const SettingsForm = Form(schema, {
  onSubmit: (values) => console.log(values),
...
| --------------- | ----------------------------------- | ----------- | -------------------------------------------------------- |
| `defaultValues` | `Record<string, unknown>`           | from schema | Initial values. Omit to derive from schema `.default()`. |
