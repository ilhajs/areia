...
Areia's `Pagination` renders page information, navigation controls, optional page-size selectors, and emits page-change events when used as an Ilha island with `Pagination.Root`.
...
```ts twoslash
import { Pagination } from "areia/components/pagination";
```

## Usage

Use `Pagination.Root` when you want interactive controls. It dispatches `pagination:page-change` and calls `setPage` / `onPageChange` when controls are used.
...
Use `Pagination(...)` for static markup or when you want to wire events yourself.
...
`Pagination.PageSize` renders a native select for choosing page size. In island mode, changing it emits `pagination:page-size-change` and calls `onPageSizeChange` on `Pagination.Root` when provided.
...
`Pagination.Root` dispatches DOM events when controls are used. Use these when you are not passing callback functions.
...
`Pagination.Root` is an Ilha island. It accepts standard HTML `div` attributes plus the following props.
