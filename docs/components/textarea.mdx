...
A multi-line text input for longer content with built-in label, description, and error support. Supports vertical resizing by default.
...
```ts twoslash
import { Textarea } from "areia/components/textarea";
```
...
For custom form layouts, use `Textarea` without `label`. Provide `aria-label` or `aria-labelledby` for accessibility.
...
export default ilha.render(
  () =>
    html`${Textarea({
      ...
      placeholder: "Type your message...",
      value:
        "This message is way too long for the field limit...",
...
export default ilha.render(
  () =>
    html`${Textarea({
      ...
      label: "Release notes",
      labelTooltip:
        "These notes are published with the release and visible to all users.",
...
export default ilha.render(
  () =>
    html`${Textarea({
      id: "review-notes",
      label: html`Notes for <strong>review</strong>`,
      required: true,
