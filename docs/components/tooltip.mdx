...
Areia's `Tooltip` is built on `@data-slot/tooltip` and rendered as an Ilha island when you use `Tooltip.Root`. It handles hover/focus interactions, warm-up behavior between nearby tooltips, collision-aware positioning, Escape dismissal, and accessible tooltip attributes.
...
```ts twoslash
import { Tooltip } from "areia/components/tooltip";
```

## Usage

Use `Tooltip.Root` for an interactive island. The root renders the tooltip markup and initializes the `@data-slot/tooltip` controller after mount.
...
For static server-rendered markup or custom initialization, use `Tooltip(...)` directly. For most application usage, prefer `Tooltip.Root`.
...
export default ilha.render(
  () =>
    html`${Tooltip.Root({
      ...
      content:
        "Areia is a vanilla TypeScript component library.",
      children: "Areia",
    })}`,
);
```
...
export default ilha.render(
  () => html`
    <div class="flex flex-wrap items-center gap-3">
      ${Tooltip.Root({
        delay: 1000,
        content: "Opens after one second",
        children: Button({ label: "1s delay" }),
      })}
      ${Tooltip.Root({
        delay: 0,
        skipDelayDuration: 0,
        content: "Opens instantly with no warm-up window",
        children: Button({ label: "Instant" }),
      })}
    </div>
  `,
);
...
The `content` prop accepts Ilha markup.
      ...
      content: html`
        <div class="flex max-w-48 flex-col gap-1">
          ...
          <span class="text-areia-subtle"
            >Creates a temporary preview environment.</span
          >
        </div>
      `,
      children: Button({ label: "Deploy" }),
    })}`,
);
```
...
`Tooltip.Root` is an Ilha island. It accepts standard HTML `div` attributes plus `@data-slot/tooltip` options.
