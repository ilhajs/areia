...
const Example = ilha.render(
  () =>
    html`${Checkbox({ label: "Accept terms and conditions" })}`,
);
// ---cut-after---
...
```ts twoslash
import { Checkbox } from "areia/components/checkbox";
```
...
const Example = ilha.render(
  () => html`${Checkbox({ label: "Remember me" })}`,
);
// ---cut-after---
Example.mount(document.getElementById("root")!);
```
...
const Example = ilha.render(
  () =>
    html`${Checkbox({ label: "Disabled", disabled: true })}`,
);
// ---cut-after---
Example.mount(document.getElementById("root")!);
```
...
const Example = ilha.render(
  () => html`${Checkbox({ "aria-label": "Select row" })}`,
);
// ---cut-after---
Example.mount(document.getElementById("root")!);
```
