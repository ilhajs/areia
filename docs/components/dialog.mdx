...
Use `Dialog.Root` for an interactive dialog. The component renders the required `dialog`, `dialog-trigger`, `dialog-portal`, `dialog-overlay`, and `dialog-content` slots for `@data-slot/dialog`.
...
You can compose slots manually when you need lower-level control. Use `Dialog.Root` at the top so the primitive is mounted.
