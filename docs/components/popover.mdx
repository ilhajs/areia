...
Areia's `Popover` is built on `@data-slot/popover` and rendered as an Ilha island when you use `Popover.Root`. It supports click triggers, dismiss on outside click or Escape, focus management, collision-aware positioning, close buttons, and optional arrows.
...
```ts twoslash
import { Popover } from "areia/components/popover";
```

## Usage

Use `Popover.Root` for an interactive island. The root renders the popover markup and initializes the `@data-slot/popover` controller after mount.
...
For static server-rendered markup or custom initialization, use `Popover(...)` directly. For most application usage, prefer `Popover.Root`.
...
export default ilha.render(
  () =>
    html`${Popover.Root({
      children: Button({ label: "Open Settings" }),
      content: html`
...
export default ilha.render(
  () => html`
    <div class="flex items-center gap-3">
      ${Popover.Root({
        align: "start",
        children: Button({ label: "Start" }),
        content: "Start aligned",
      })}
      ${Popover.Root({
        align: "center",
        children: Button({ label: "Center" }),
        content: "Center aligned",
      })}
      ${Popover.Root({
        align: "end",
        children: Button({ label: "End" }),
        content: "End aligned",
      })}
    </div>
  `,
...
export default ilha.render(
  () =>
    html`${Popover.Root({
      children: Button({ label: "User Profile" }),
      content: html`
...
export default ilha.render(
  () =>
    html`${Popover.Root({
      defaultOpen: true,
      children: Button({ label: "Open by default" }),
...
export default ilha.render(
  () =>
    html`${Popover.Root({
      ...
      children: Button({ label: "Persistent" }),
      content: html`
...
`Popover.Root` is an Ilha island. It accepts standard HTML `div` attributes plus `@data-slot/popover` options.
